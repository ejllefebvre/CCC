...
public class Round {
	...
	public Round(ArrayList<Candidate> candidates, ArrayList<Vote> input) {
		master = input;
		...
		if(winner) {
			...
			System.out.println();
			System.out.println();
			new Round(this.getNextRoundCandidates(), this.getNextRoundList());
		}
	}
	...
	public void redistribute() {
		int index = findMinimum(byCandidate);
		
		for(Vote vote: byCandidate.get(index).getVotes()) {
			vote.remove(0);
		}
	...
	public int findMinimum(ArrayList<Candidate> candidates) {
		int minimumVotes = candidates.get(0).getVoteScore();
		int minimumIndex = 0;
